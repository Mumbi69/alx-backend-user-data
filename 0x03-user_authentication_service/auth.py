...
def _hash_password(password: str) -> bytes:
    """Represents a salted hash of the input password"""
...
class Auth:
    ...
    def register_user(self, email: str, password: str) -> User:
        """represents a new user with the provided email and password."""
        try:
    ...
    def valid_login(self, email: str, password: str) -> bool:
        """This function validates user login credentials."""
        try:
    ...
    def destroy_session(self, user_id: str):
        """This function destroys user session based on
        the provided user ID"""
        if user_id:
            self._db.update_user(user_id, session_id=None)

    def get_reset_password_token(self, email: str) -> str:
        """This function generates and retrieve a reset
        password token for the user."""
        try:
            user = self._db.find_user_by(email=email)
        except NoResultFound:
            raise ValueError()
